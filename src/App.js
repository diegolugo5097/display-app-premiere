...
function App() {
  ...
  const [ambientAudio, setAmbientAudio] = useState("/fondo-ambiental.mp3");
  ...
  useEffect(() => {
    const onSetHero = ({ hero: h }) => {
      ...
      // ✅ Audio especial para DARTH VADER
      if ((h?.name || "").toUpperCase() === "DARTH VADER") {
        setAmbientAudio("/darth-vader.mp3");
      } else {
        setAmbientAudio("/fondo-ambiental.mp3");
      }
  ...
  useEffect(() => {
    const ambientSound = /** @type {HTMLAudioElement | null} */ (
      document.getElementById("ambientSound")
    );
    const pulseSound = /** @type {HTMLAudioElement | null} */ (
      document.getElementById("pulseSound")
    );
    if (!ambientSound || !pulseSound) return;
    ...
    // 👇 se vuelve a disparar si cambia el archivo de ambientAudio
  }, [soundEnabled, ambientAudio]);
    ...
    <div style={styles.screen}>
      <div style={styles.screen}>
        ...
        <HUDOverlay
          ...
          ambientAudio={ambientAudio} // <-- 🔊 se pasa como prop
        />
        ...
        {infoVisible && (
          <RightInfoPanel
            hero={hero}
            accentColor={accentColor}
            animateIn={animateInfo}
          />
        )}

        {/* ECG */}
        <ECGPanel
          heartRate={hero.medical.heartRate}
          status={hero.medical.status}
          accentColor={accentColor}
        />
        ...
        <button
          ...
          style={{
            ...
            right: "1rem",
            zIndex: 1,
            backgroundColor: soundEnabled ? "#052e16" : "#2a0a0a",
            color: soundEnabled ? "#4ade80" : "#ef4444",
            border: `1px solid ${soundEnabled ? "#4ade80" : "#ef4444"}`,
            borderRadius: ".5rem",
            padding: ".6rem 1rem",
            fontSize: ".65rem",
            fontWeight: 700,
            letterSpacing: ".1em",
            cursor: "pointer",
            boxShadow: `0 0 20px ${soundEnabled ? "#4ade80aa" : "#ef4444aa"}`,
          }}
        >
          {soundEnabled ? "🔊 SONIDO ACTIVADO" : "🔇 SONIDO MUTEADO"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div style={styles.radiationGraphBox}>
      ...
      <div style={styles.radiationLegend}>
        <div style={styles.radRow}>
          <span style={styles.radLabel}>RAD LVL</span>
          <span style={{ ...styles.radValue, color }}>VIVO</span>
        </div>
        <div style={styles.radRow}>
          <span style={styles.radLabel}>ESTADO</span>
          <span style={{ ...styles.radValue, color }}>ESTABLE</span>
        </div>
      </div>
    </div>
  );
...
function HUDOverlay({ hero, dangerMode, accentColor, ambientAudio }) {
  return (
    <div style={styles.hudOverlay}>
      ...
      {/* 🔊 Aquí ya llega el ambientAudio correcto */}
      <audio id="ambientSound" src={ambientAudio} loop />
        ...
        <div style={styles.hudLineRow}>
          ...
          <div
            style={{
              ...styles.hudValue,
              color: accentColor,
              textShadow: `0 0 8px ${accentColor}aa`,
            }}
          >
            {hero.medical.threatLevel || "—"}
          </div>
  ...
  return (
    <div
      style={{
        ...
        boxShadow: `0 0 20px ${accentColor}55 inset`,
      }}
    >
      ...
      <div style={styles.ecgReadout}>
        <div style={styles.readoutRow}>
          <span style={styles.readoutLabel}>HR</span>
          <span style={styles.readoutValue}>{heartRate || "—"}</span>
        </div>
        <div style={styles.readoutRow}>
  ...
  return (
    <div
      style={{
        ...
        ...(animateIn ? styles.rightPanelVisible : styles.rightPanelHidden),
      }}
    >
      <div
        style={{
          ...styles.rightPanelCard,
          borderColor: `${accentColor}66`,
          boxShadow: `0 0 30px ${accentColor}66, 0 0 100px ${accentColor}22`,
          animation: "hologramIn 0.8s ease-out, pulseGlow 2s infinite",
        }}
      >
        ...
        <div style={styles.storyTextSmall}>
          {hero.story || "Sin registros disponibles."}
        </div>
        ...
        <InfoRowMini
          ...
          value={hero.debutYear ? `${hero.debutYear}` : "—"}
        />
        <InfoRowMini
          label="EDAD / ESTADO"
          value={hero.currentAge ? `${hero.currentAge} AÑOS` : "—"}
        />
        ...
        <div style={styles.medNotesSmall}>
  ...
  return (
    <div style={styles.bioRow}>
      ...
      <div
        style={{
          ...styles.bioValue,
          color: accentColor && value ? accentColor : "#fff",
          textShadow:
            accentColor && value
              ? `0 0 8px ${accentColor}aa`
...
const styles = {
  ...
  bioTitleSection: {
    fontWeight: 700,
    color: "#fff",
    textShadow: "0 0 8px rgba(255,255,255,.2)",
    fontSize: ".6rem",
    letterSpacing: ".2em",
  },
